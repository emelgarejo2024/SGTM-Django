...
def agenda(request):
    ...
    bloques = ReservaFacade.obtener_bloques_disponibles(especialidad_id, fecha)
...
def confirmar_reserva(request):
    ...
    if request.method == 'POST':
        ...
        # SIMULACIÓN DE LOGIN 
        # Tomamos el primer paciente que exista en la Base de Datos (del archivo datos_prueba.json)
        paciente_simulado = Usuario.objects.filter(rol='PACIENTE').first()
        
        if not paciente_simulado:
            messages.error(request, "Error interno: No hay pacientes registrados en el sistema.")
            return redirect('index')

        # Procesamos la reserva atómica integrando la consulta de Edgar
        exito, mensaje = ReservaFacade.procesar_reserva_atomica(bloque_id, paciente_simulado)
