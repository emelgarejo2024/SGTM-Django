...
class TestVistas:
    ...
    def test_index_incluye_especialidades_en_contexto(self):
        ...
        nombres = list(response.context['especialidades'].values_list('nombre', flat=True))
    ...
    def test_agenda_filtra_por_especialidad(self):
        """La vista agenda filtra bloques por especialidad cuando se provee el parámetro."""
        medico = UsuarioFactory.crear_usuario('MEDICO', '10101010-1', 'dr_ag', '123', 'L', 'M', 'l@cl.cl')
        ...
        BloqueDisponibilidad.objects.create(
            ...
            fecha=manana, hora_inicio=time(8, 0), hora_fin=time(8, 30)
        )
        response = self.client.get(reverse('agenda'), {'especialidad': esp.id, 'fecha': str(manana)})
        assert response.status_code == 200
    ...
    def test_agenda_sin_especialidad_muestra_todas(self):
        """Sin parámetros, la vista agenda muestra todos los bloques disponibles."""
        medico = UsuarioFactory.crear_usuario('MEDICO', '20202020-2', 'dr_ag2', '123', 'X', 'Y', 'xy@cl.cl')
    ...
    def test_api_especialidades_devuelve_json(self):
        """La API JSON de especialidades retorna status 200 y datos correctos."""
    ...
    def test_confirmar_reserva_sin_pacientes_redirige(self):
        """Si no hay pacientes en la BD, confirmar_reserva redirige a index con error."""
    ...
    def test_confirmar_reserva_exitosa_redirige_a_index(self):
        """Con un paciente y bloque válidos, la reserva se crea y redirige a index."""
        paciente = UsuarioFactory.crear_usuario('PACIENTE', '30303030-3', 'pac_view', '123', 'V', 'W', 'vw@cl.cl')
        medico = UsuarioFactory.crear_usuario('MEDICO', '40404040-4', 'dr_view', '123', 'Dr', 'V', 'drv@cl.cl')
        ...
        bloque = BloqueDisponibilidad.objects.create(
            medico=medico, especialidad=esp,
            fecha=manana, hora_inicio=time(15, 0), hora_fin=time(15, 30)
        )
        response = self.client.post(reverse('confirmar_reserva'), {'bloque_id': bloque.id})
        assert response.status_code == 302
        assert response['Location'].endswith(reverse('index'))
