...
class TestReservaFacade:
    ...
    def setup_datos(self):
        # Esta función "fixture" prepara los datos base para no repetir código en cada test
        paciente = UsuarioFactory.crear_usuario('PACIENTE', '11111111-1', 'pac_test', '123', 'Juan', 'A', 'j@cl.cl')
        medico = UsuarioFactory.crear_usuario('MEDICO', '22222222-2', 'med_test', '123', 'Dr', 'B', 'm@cl.cl')
        especialidad = Especialidad.objects.create(nombre='Cardiología')
        
        # Creamos un bloque para mañana (asegurando que no sea sábado ni domingo para no chocar con lógicas de negocio)
    ...
    def test_crear_reserva_exitosa(self, setup_datos):
        ...
        exito, mensaje = ReservaFacade.procesar_reserva_atomica(bloque_id=bloque.id, paciente=paciente)
    ...
    def test_falla_por_overbooking(self, setup_datos):
        ...
        paciente2 = UsuarioFactory.crear_usuario('PACIENTE', '33333333-3', 'pac2', '123', 'Pedro', 'C', 'p@cl.cl')
        
        # 1. El primer paciente toma el bloque exitosamente
        exito1, mensaje1 = ReservaFacade.procesar_reserva_atomica(bloque_id=bloque.id, paciente=paciente1)
        assert exito1 is True
        
        # 2. El segundo paciente intenta tomar el mismo bloque exacto
        exito2, mensaje2 = ReservaFacade.procesar_reserva_atomica(bloque_id=bloque.id, paciente=paciente2)
        assert exito2 is False
        assert "El bloque ya fue tomado" in mensaje2
            
        # 3. Verificamos que la base de datos protegió la integridad (solo debe haber 1 reserva)
        assert Reserva.objects.count() == 1
    ...
    def test_falla_por_bloque_inexistente(self, setup_datos):
        ...
        exito, mensaje = ReservaFacade.procesar_reserva_atomica(bloque_id=999, paciente=paciente)
...
class TestReservaFacadeConsultas:
    ...
    def datos_consulta(self):
        medico = UsuarioFactory.crear_usuario('MEDICO', '99999999-9', 'dr_cons', '123', 'Dra', 'C', 'drc@cl.cl')
        ...
        bloque1 = BloqueDisponibilidad.objects.create(
            medico=medico, especialidad=esp1,
            fecha=manana, hora_inicio=time(9, 0), hora_fin=time(9, 30)
        )
        bloque2 = BloqueDisponibilidad.objects.create(
            medico=medico, especialidad=esp2,
            fecha=pasado, hora_inicio=time(11, 0), hora_fin=time(11, 30)
        )
    ...
    def test_obtener_bloques_disponibles_sin_filtros(self, datos_consulta):
        """Cubre rama sin filtros de obtener_bloques_disponibles — líneas 59, 66."""
    ...
    def test_obtener_bloques_disponibles_filtrado_por_especialidad(self, datos_consulta):
    ...
    def test_obtener_bloques_no_incluye_ocupados(self, datos_consulta):
        """Verifica que bloques con esta_disponible=False no aparecen en el listado."""
        *_, bloque1, bloque2, manana, pasado = datos_consulta
